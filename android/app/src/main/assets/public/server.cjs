    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            currentPrice: { type: import_genai.Type.NUMBER, description: "Current average price per kg" },
            currency: { type: import_genai.Type.STRING, description: "Currency unit e.g., TZS, KES" },
            priceTrend: { type: import_genai.Type.STRING, description: "Visual trend direction: 'Up', 'Down', or 'Stable'" },
            analysis: { type: import_genai.Type.STRING, description: "Brief summary of the current market state" },
            forecast: {
              ...
              items: {
                ...
                properties: {
                  month: { type: import_genai.Type.STRING, description: "Name of the month" },
                  price: { type: import_genai.Type.NUMBER, description: "Forecasted price per kg in currency specified" }
                }
              }
            },
            risks: {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            diagnosis: { type: import_genai.Type.STRING, description: "Name/brief description of the suspected disease or pest" },
            severity: { type: import_genai.Type.NUMBER, description: "Severity indicator from 1 to 10" },
            organicControl: {
              type: import_genai.Type.ARRAY,
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            seasonalInsights: { type: import_genai.Type.STRING, description: "Aggregated regional weather and planting conditions" },
            recommendedCrops: {
              type: import_genai.Type.ARRAY,
              items: {
                type: import_genai.Type.OBJECT,
                ...
                properties: {
                  cropName: { type: import_genai.Type.STRING },
                  demandLevel: { type: import_genai.Type.STRING, description: "High, Medium, or Emerging" },
                  avgPricePerKg: { type: import_genai.Type.STRING, description: "Price indicator e.g. 2,500 TZS" },
                  whyRecommended: { type: import_genai.Type.STRING, description: "Why this fits current soil/demand indicators" }
                }
              }
            },
            demandCalendar: {
              ...
              items: {
                ...
                properties: {
                  ...
                  activity: { type: import_genai.Type.STRING, description: "Activity e.g., Sell, Plant, Harvest" },
                  cropInterest: { type: import_genai.Type.STRING, description: "Crop of interest during this window" }
                }
              }
            }
          }
...
function createSimulatedPricePrediction(cropName, region, language) {
  const isSw = language === "sw";
  const basePrice = cropName.toLowerCase().includes("maize") || cropName.toLowerCase().includes("mahindi") ? 1200 : cropName.toLowerCase().includes("rice") || cropName.toLowerCase().includes("mchele") ? 2200 : cropName.toLowerCase().includes("coffee") || cropName.toLowerCase().includes("kahawa") ? 4500 : cropName.toLowerCase().includes("tomato") || cropName.toLowerCase().includes("nyanya") ? 1800 : 2500;
  ...
  return {
    ...
    currency: "TZS",
    priceTrend: "Up",
    analysis: isSw ? `Uchambuzi wa ${cropName} katika mkoa wa ${region || "Afrika Mashariki"}: Ugavi umepungua kwa sababu ya mfumo wa mvua wa hivi karibuni, ambayo huenda ikaongeza bei kwa 15% katika miezi mitatu ijayo.` : `Market analysis for ${cropName} in ${region || "East Africa"}: Reduced seasonal supply is expected to trigger a 15% price increase over the next three months as demand remains high.`,
...
function createSimulatedPestDiagnosis(cropName, symptoms, language) {
  const isSw = language === "sw";
  ...
  return {
    diagnosis: isInsect ? isSw ? "Minyoo ya Kuvamia (Fall Armyworm) au Wadudu wa Majani" : "Fall Armyworm Defoliation" : isSw ? "Ugonjwa wa Kuvu mwekundu / Bakteria ya Majani" : "Leaf Rust Fungus (Puccinia sorghi)",
...
function createSimulatedDemandRecommendation(currentCrops, region, language) {
  const isSw = language === "sw";
  return {
    seasonalInsights: isSw ? `Tathmini ya mkoa wa ${region || "Afrika Mashariki"}: Kipindi cha sasa kina mvua za wastani na miale mizuri ya jua, inayofaa sana kwa mboga na matunda ya muda mfupi ya kibiashara.` : `Regional Assessment for ${region || "East Africa"}: Current cycle trends exhibit moderate rainfall with strong solar exposure, presenting ideal agro-climatic conditions for short-rotation vegetable and cash legumes.`,
    recommendedCrops: [
      {
        ...
        whyRecommended: isSw ? "Mahitaji makubwa katika soko la mijini huku kukiwa na uhaba mkubwa wa wazalishaji wa dhati msimu huu." : "Urban wholesale demand remains consistently high while local production is seasonally constrained, offering high profit margins."
      },
      {
        ...
        whyRecommended: isSw ? "Mazao haya husaidia kuongeza nitrojeni ya asili kwenye udongo wako huku ununuzi wa pamoja ukiwa umehakikishwa." : "Serves as an exceptional companion crop for your fields, restoring soil nitrogen while maintaining locked buyer buyback contracts."
      },
      {
        ...
        whyRecommended: isSw ? "Inafaa sana kwa ajili ya usambazaji wa karibu mjini. Muda wa kukomaa ni mfupi sana (siku 75-90)." : "Perfect fast-turnaround crop (75-90 days maturity) targeting the high-volume metropolitan fresh-markets."
      }
    ],
    demandCalendar: [
      {
